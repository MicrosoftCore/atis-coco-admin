...
export default {
  data () {
    return {
      url: this.$urls._('manage.farmer.getmanagefarmerlist'),
      StatusMap: {
        '0': '未认证',
        '1': '审核中',
        '2': '未通过',
  ...
  computed: {
    ...
    columnOptions () {
      return [
        ...
        { label: '体检次数', prop: 'DetectionCount' },
        {
          label: '资料完成度',
          prop: 'IsCompletedCropInfo',
          render: (h, scope) => {
            if (scope.IsCompletedCropInfo) {
              return h(
                'span',
                '已完成'
              )
