  ...
  <el-container style="border: 1px solid #eee; background-color:#fff;height:calc(100% - 60px);">
    ...
    <el-main style="padding:20px;">
      ...
      <div class="img-con" ref="images" v-viewer>
        ...
        <div v-for="(section, pindex) in imagesSection" :key="pindex">
          <div class="item" v-if="orderInfo[section.prop].length">
            ...
            <ul class="images img-list dd-clearfix">
               <el-checkbox-group 
    v-model="checkedImage[section.prop]"
    :min="1"
    :max="9">
                <li class="img-item" v-for="(media, index) in orderInfo[section.prop]" :key="index">
                  ...
                  <span class="playvideo" v-else-if="media.Type === 2" @click="videoSrc = media.ImageUrl"></span>
...
export default {
  ...
  data () {
    return {
      ...
      checkedImage: {
        ImageListScene: [0],
        ImageListLeafSurface: [0],
        ImageListSoilRoots: [0],
        ImageListPharmacy: [0],
        ImageListOther: [0]
      },
      listDetectionId: undefined
    }
  },
  computed: {
    ...mapState('common', {
  ...
  methods: {
    ...
    getOrderInfo () {
      ...
      this.listDetectionId = Ids
      if (Ids) {
        this.$axios.post(this.$urls._('bench.detection.getmergedetection'), Ids).then(resp => {
          this.orderInfo = resp
        }).catch(e => {
          this.back()
        })
    ...
    submit (ApproveFlag) {
      if (!this.submitting) {
        let imageList = []
        if (this.orderInfo.ImageListScene.length > 0) {
          this.checkedImage.ImageListScene.forEach(element => {
            imageList.push(this.orderInfo.ImageListScene[element])
          })
        }
        if (this.orderInfo.ImageListLeafSurface.length > 0) {
          this.checkedImage.ImageListLeafSurface.forEach(element => {
            imageList.push(this.orderInfo.ImageListLeafSurface[element])
          })
        }
        if (this.orderInfo.ImageListSoilRoots.length > 0) {
          this.checkedImage.ImageListSoilRoots.forEach(element => {
            imageList.push(this.orderInfo.ImageListSoilRoots[element])
          })
        }
        if (this.orderInfo.ImageListPharmacy.length > 0) {
          this.checkedImage.ImageListPharmacy.forEach(element => {
            imageList.push(this.orderInfo.ImageListPharmacy[element])
          })
        }
        if (this.orderInfo.ImageListOther.length > 0) {
          this.checkedImage.ImageListOther.forEach(element => {
            imageList.push(this.orderInfo.ImageListOther[element])
          })
        }
        this.submitting = true
        this.$axios.post(this.$urls._('bench.detection.mergedetectionfirst'), {
          ApproveFlag,
          DetectionId: this.orderInfo.DetectionOrderId,
          DetectionIdList: [this.orderInfo.DetectionOrderId],
          ListDetectionId: this.listDetectionId,
...
.img-con {
  ...
  .img-item {
    ...
    .playvideo {
      display: inline-block;
      background: url('../../../../assets/image/playVideo.png') center center no-repeat;
