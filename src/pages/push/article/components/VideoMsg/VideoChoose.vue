<template>
  <el-dialog title="选择视频" :visible.sync="visible" width="960px">
    <div v-if="noRecords" style="padding:20px;">暂无可推送的视频</div>
    <div class="box-wrapper" v-else v-loading="loading">
      <div class="box" v-for="(item, index) in lists" :key="index" @click="onChoose(item)">
        <section class="aaa">
          <div>{{ item.Title }}</div>
          <div>{{ item.UpdateTime | moment('MMMDo') }}</div>
          <video alt :src="item.Content" v-if="item.Content"></video>
